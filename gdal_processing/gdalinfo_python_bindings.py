...
def get_data(path, file):
    ...
    # Data Extraction
    #
    dataset = gdal.Open(path + file, gdal.GA_ReadOnly)
...
if __name__ == '__main__':
    #
    # Output using gdalinfo
    #
    sar_path = "/home/ryan/Downloads/Dataset/S1A_IW_GRDH_1SDV_20180406T133350_20180406T133415_021347_024BD7_6B09.SAFE"\
               "/measurement/"
    sar_file = "s1a-iw-grd-vv-20180406t133350-20180406t133415-021347-024bd7-001.tiff"
    output_file = "test-gdal.tiff"
    bash_cmd = "gdalwarp -t_srs EPSG:4326 -ot Float32 " + sar_path + sar_file + " " + sar_path + output_file
    # os.system(bash_cmd)
    os.system("gdalinfo " + sar_path + output_file)
    print("#############################################################################################################\n")

    #
    # Output using gdal python bindings
    #
    data = get_data(sar_path, output_file)
    gdalinfo_python(data)
