...
{ 
 ...
 echo " "
 echo "   -A <folder>                   : save the log folder, OSquery-result.xml, and products-list.csv to a specified (aim/target/destination) folder."
 echo " "
 echo " "
...
{
	echo "CUSTOM ECHO: download_product function has been entered."
	echo "CUSTOM ECHO: $1"
	...
	if echo $prod5P  | grep -q "$mission5P"; then
		${WC} ${AUTH} ${TRIES} --progress=dot -e dotbytes=10M -c --output-file=$aim_folder/logs/log.${3}.log -O $output_folder/${3}".nc" "${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/\$value";
	else
		echo "CUSTOM ECHO: Entered the else block of the S5P product block."
        ${WC} ${AUTH} ${TRIES} --show-progress --progress=bar:force:noscroll -c --output-file=$aim_folder/logs/log.${3}.log -O $output_folder/${3}".zip" "${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/\$value";
	fi
	test=$?;
	...
	#------------------------------------------------------------------------------
	http_response=`cat $aim_folder/logs/log.${3}.log| grep  '\''403 Forbidden\|202 Accepted\|503 Service Unavailable'\''`;
	...
	if [ ! -z "$http_response" ]; then
		...
		if [ $test -eq 0 ]; then 
			echo "Product ${3} successfully downloaded at " `tail -2 $aim_folder/logs/log.${3}.log | head -1 | awk -F"(" '\''{print $2}'\'' | awk -F")" '\''{print $1}'\''`;
...
while getopts ":A:c:C:d:D:e:E:f:F:h:i:l:L:m:n:N:o:O:p:P:q:r:R:s:S:t:T:u:V:w:W:" opt; do
	case $opt in
		d)
		...
		O)
			...
			;;
		A)
			export aim_folder="$OPTARG"
			export AIM_FOLDER_IS_SET='OK'
			;;
		N)
...
mkdir -p "$aim_folder/logs/"

if [ ! -z $check_retry ] && [ -s $FAILED_retry ]; then
	echo "CUSTOM ECHO: Entered failed_retry block"
	...
	cat ${INPUT_FILE} | xargs -n 4 -P ${THREAD_NUMBER} sh -c ' while : ; do
	    echo "Downloading product ${3} from link ${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/\$value";
	    ${WC} ${AUTH} ${TRIES} --progress=dot -e dotbytes=10M -c --output-file=$aim_folder/logs/log.${3}.log -O $output_folder/${3}".zip" "${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/\$value";
	    test=$?;
	    if [ $test -eq 0 ]; then
	            echo "Product ${3} successfully downloaded at " `tail -2 $aim_folder/logs/log.${3}.log | head -1 | awk -F"(" '\''{print $2}'\'' | awk -F")" '\''{print $1}'\''`;
	            remoteMD5=$( ${WC} -qO- ${AUTH} ${TRIES} -c "${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/Checksum/Value/$value" | awk -F">" '\''{print $3}'\'' | awk -F"<"     '\''{print $1}'\'');
...
if [[ $CHECK_VAR == true &&  ! -z $AIM_FOLDER_IS_SET ]]; then
    echo "'-A option' is set to $aim_folder. Product downloads are stored in $aim_folder. "
    echo ""
fi
...
START=$((PAGE-1))
START=$((START*ROWS))
export INIT_QUERY_STATEMENT=$QUERY_STATEMENT
export QUERY_STATEMENT="${DHUS_DEST}/search?q="${INIT_QUERY_STATEMENT}"&rows="${ROWS}"&start="${START}""
...
PAGE_COUNT=1
if [ -z $NAMEFILERESULTS ]; then
	export NAMEFILERESULTS="$aim_folder/OSquery-result-$PAGE_COUNT.xml"
fi
if [ -z $PRODUCTLIST ]; then
   export PRODUCTLIST="$aim_folder/products-list-$PAGE_COUNT.csv"
fi
/bin/rm -f $NAMEFILERESULTS
/bin/rm -f "$aim_folder/product_list"
/bin/rm -f "$aim_folder/products-list.csv"

${WC} ${AUTH} ${TRIES} -c -O "${NAMEFILERESULTS}" "${QUERY_STATEMENT}"
LASTDATE=`date -u +%Y-%m-%dT%H:%M:%S.%NZ`
sleep 5

export TOTAL_RESULTS="$(cat "${NAMEFILERESULTS}" | grep '<opensearch:totalResults>' | cut -f2 -d'>' | cut -f1 -d'<')"
echo "CUSTOM ECHO: Total Results = ${TOTAL_RESULTS}"

PAGE_COUNT=1
while [ $START -le $TOTAL_RESULTS ]; do
	echo "CUSTOM ECHO: Page = $PAGE_COUNT"
	cat "${NAMEFILERESULTS}" | grep '<id>' | tail -n +2 | cut -f2 -d'>' | cut -f1 -d'<' | cat -n > .product_id_list
	awk -v var="$START" '{$1 += var; printf "%6s %37s\n", $1, $2}' .product_id_list > /tmp/product_id_list && mv /tmp/product_id_list .product_id_list
	...
	awk -v var="$START" '{$1 += var; printf "%6s %68s\n", $1, $2}' .product_title_list > /tmp/product_title_list
	...
	paste -d\\n .product_id_list /tmp/product_title_list | sed 's/[",:]/ /g' >> "$aim_folder/product_list"
	cat .product_title_list .product_link_list | sort -k1n,1 -k2r,2 | sed 's/[",:]/ /g' | sed 's/https \/\//https:\/\//' > .product_list_withlink
	rm -f .product_id_list /tmp/product_id_list .product_link_list .product_title_list /tmp/product_title_list .product_ingestion_time_list
	...
	START=$((START+ROWS))
	((PAGE_COUNT++))
	if [ $START -le $TOTAL_RESULTS ]; then
		echo ""
		export QUERY_STATEMENT="${DHUS_DEST}/search?q="${INIT_QUERY_STATEMENT}"&rows="${ROWS}"&start="${START}""
		echo "HTTP request done: "$QUERY_STATEMENT""
		echo ""
...
((PAGE_COUNT--))
cat $aim_folder/OSquery-result-*.xml > "$aim_folder/OSquery-result.xml"
cat $aim_folder/products-list-*.csv > "$aim_folder/products-list.csv"
while [ $PAGE_COUNT -gt 0 ]; do
	rm "$aim_folder/OSquery-result-$PAGE_COUNT.xml"
	rm "$aim_folder/products-list-$PAGE_COUNT.csv"
	((PAGE_COUNT--))
done
...
if [ "${TO_DOWNLOAD}" == "manifest" -o "${TO_DOWNLOAD}" == "all" ]; then
	echo "CUSTOM ECHO: Entered the manifest if block"
	...
	cat ${INPUT_FILE} | xargs -n 4 -P ${THREAD_NUMBER} sh -c 'while : ; do
		...
		${WC} ${AUTH} ${TRIES} --progress=dot -e dotbytes=10M -c --output-file=$aim_folder/logs/log.${3}.log -O ./MANIFEST/manifest.safe-${3} "${DHUS_DEST}/odata/v1/Products('\''"$1"'\'')/Nodes('\''"$3".SAFE'\'')/Nodes('\'manifest.safe\'')/\$value" ;
		test=$?;
		if [ $test -eq 0 ]; then
			echo "Manifest ${3} successfully downloaded at " `tail -2 $aim_folder/logs/log.${3}.log | head -1 | awk -F"(" '\''{print $2}'\'' | awk -F")" '\''{print $1}'\''`;
		fi;
		[[ $test -ne 0 ]] || break;
	done '
fi
...
if [ "${TO_DOWNLOAD}" == "product" -o "${TO_DOWNLOAD}" == "all" ]; then
	echo "CUSTOM ECHO: Entered the product if block"
    export INPUT_FILE="$aim_folder/product_list"
	mkdir -p $output_folder
	mkdir -p $aim_folder
